class FetchNewArticlesJob < ApplicationJob
  ...
  def existing_article?(item)
    recent_articles.select { |article| article.link == item.link }.first.present?
  end

  def recent_articles
    @recent_articles ||= Article.select(:link).where('published_date > ?', Time.now - 6.hours)
  end
  ...
  def latest_entry_date(source_id)
    latest_published_articles.select { |item| item.source_id == source_id }.first&.date || Time.now - 1.year
  end
